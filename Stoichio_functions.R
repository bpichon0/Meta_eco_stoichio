...
colors_ecosystem <- c("Aquatic" = "#90D0E0", "Terrestrial" = "#7AA969")
color_resources <- c("N" = "#90D0E0", "C" = "brown")
...
Get_initial_values <- function(param, rD_ini = .1, value_given = F, val = 2) {
    if (value_given) {
        ...
        state <- c("H" = 2, "HN" = 0, "C" = 2, "CN" = 0, "P" = 3, "PN" = 0, "B" = 2, "BN" = 0, "DCt" = 3, "DNt" = 0, "DCa" = 3, "DNa" = 0, "Nt" = 2, "Na" = 2)
    }
    state[c("DNt")] <- rD_ini * state[c("DCt")]
    state[c("DNa")] <- rD_ini * state[c("DCa")]
    state["HN"] <- param$rH * state["H"]
    state["CN"] <- param$rC * state["C"]
    state["PN"] <- param$rP * state["P"]
...
Get_trophic_level <- function(vector) {
    return(unlist(lapply(vector, function(x) {
        if (x %in% c("Plants_N", "Plants_C", "Decomposers_N", "Decomposers_C")) {
            return("Producers")
        }
        if (x %in% c("Herbivores_C", "Consumers_C", "Herbivores_N", "Consumers_N")) {
            return("Consumers")
        }
        if (x %in% c("Top_cons_H_C", "Top_cons_H_N", "Top_cons_C_C", "Top_cons_C_N")) {
            return("Top predators")
        }
        if (x %in% c("Nitrogen_T_N", "Nitrogen_A_N")) {
            return("Nitrogen")
        } else {
            return("Detritus")
        }
...
Primary_productivity <- function(state, param, consumers = F, colim = F) {
    ...
    return(list(
        Terrestrial = state$aP * state$Nitrogen_T_N,
        Aquatic = prod_aqua, Aqua_tot = prod_tot_aq
    ))
}
...
Secondary_productivity <- function(state, param, consumers = F) {
    ...
    return(list(
        Terrestrial = state$eH * state$aH * state$Plants_C,
...
Secondary_productivity_topconsum <- function(state, param) {
    ...
    return(list(
        Terrestrial = state$eTH * state$aTH * state$Herbivores_C,
        Aquatic = state$eTC * state$aTC * state$Consumers_C
    )) # constant
}
...
Secondary_production_topconsum <- function(state, param) {
    ...
    return(list(
        Terrestrial = state$eTH * state$aTH * state$Top_cons_H_C * state$Herbivores_C,
...
Compute_feedbacks <- function(Eq, param, type_prod = "Production", n_time = 1000,
    ...
    if (top_consumers) {
        state <- State_topconsum(state, param_b)
    }
    ...
    if (top_consumers == T) {
        ...
        return(list(
            Net_T_1 = Net_T_1, Net_A_1 = Net_A_1, Net_T_2 = Net_T_2, Net_A_2 = Net_A_2, Net_T_3 = Net_T_3, Net_A_3 = Net_A_3,
        ...
        return(list(
            Net_T_1 = Net_T_1, Net_A_1 = Net_A_1, Net_T_2 = Net_T_2, Net_A_2 = Net_A_2,
            P1_no_A_to_T = P1_no_A_to_T$Terrestrial, P1_no_T_to_A = P1_no_T_to_A$Aquatic, P2_no_A_to_T = P2_no_A_to_T$Terrestrial, P2_no_T_to_A = P2_no_T_to_A$Aquatic,
            Eq_uni_A = Eq_no_A_to_T, Eq_uni_T = Eq_no_T_to_A
        ))
    }
...
Net_flow_nitrogen <- function(data, param) {
    ...
    netflow <- tibble(
        N_receive_A = N_receive_A, C_receive_A = C_receive_A,
...
Plot_net_ecosystem_flow <- function(net_flow_C_to_A, net_flow_N_to_A, vertex_size = 50, scaling_width = 4) {
    transform_C_width <- (scaling_width * net_flow_C_to_A / 0.8874744) + 2
    transform_N_width <- (scaling_width * net_flow_N_to_A / 0.8874744) + 2

    matrix <- matrix(c(0, 0, 0, 0), nrow = 2, byrow = F)
    colnames(matrix) <- rownames(matrix) <- c("A", "T")
    net <- graph_from_adjacency_matrix(matrix, mode = "directed", diag = F)
    V(net)$label <- c("A", "T")
    V(net)$size <- 50
    V(net)$frame.color <- c("black")

    if (net_flow_C_to_A > 0 & net_flow_N_to_A > 0) {
        net <- net %>% add_edges(., c("T", "A", "T", "A"))
        E(net)$color <- c("#90D0E0", "#B17F4F")
        plot(net,
            vertex.color = c(col.alpha("#3023C7", .7), col.alpha("#21AB10", .5)), label.color = "white", vertex.label.color = "white",
            edge.arrow.width = c(2), edge.arrow.size = .5, edge.width = c(0, transform_N_width, transform_C_width)
        )
    }

    if (net_flow_C_to_A < 0 & net_flow_N_to_A > 0) {
        net <- net %>% add_edges(., c("T", "A", "A", "T"))
        E(net)$color <- c("#90D0E0", "#B17F4F")
        plot(net,
            vertex.color = c(col.alpha("#3023C7", .7), col.alpha("#21AB10", .5)), label.color = "white", vertex.label.color = "white", edge.curved = 0.8,
            edge.arrow.width = c(2), edge.arrow.size = .75, c(transform_N_width, transform_C_width)
        )
    }

    if (net_flow_C_to_A < 0 & net_flow_N_to_A < 0) {
        net <- net %>% add_edges(., c("A", "T", "A", "T"))
        E(net)$color <- c("#B17F4F", "#90D0E0")
        plot(net,
            vertex.color = c(col.alpha("#3023C7", .7), col.alpha("#21AB10", .5)), label.color = "white", vertex.label.color = "white",
...
Plot_meta_ecosystem_graph <- function(data_save, data, param) {
    ...
    # two ecosystems
    rectangle <- tibble(
        xmin = c(0.5 - filter(circle, name == "Nt")$r - .1, 2.5 - filter(circle, name == "Na")$r - .1),
    ...
    ggplot() +
        geom_rect(data = rectangle, mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = name), color = "transparent", alpha = 0.5) +
        ...
        geom_segment(
            data = arrow_line_recy, aes(x = x, y = y, xend = xend, yend = yend, linetype = linetype, color = color), arrow = arrow(length = unit(0.2, "cm")),
            linetype = 1, color = "gray70", size = .1
        ) +
        geom_segment(
            data = arrow_line_troph, aes(x = x, y = y, xend = xend, yend = yend, linetype = linetype, color = color), arrow = arrow(length = unit(0.2, "cm")),
            size = .7, linetype = 1, color = "black"
        ) +
        geom_curve(
            data = arrow_curve_recy, aes(x = x, y = y, xend = xend, yend = yend, linetype = linetype, color = color), arrow = arrow(length = unit(0.2, "cm")),
            curvature = 0.2, size = .1, linetype = 1, color = "gray70"
        ) +
        geom_curve(
            data = arrow_curve_troph, aes(x = x, y = y, xend = xend, yend = yend, linetype = linetype, color = color), arrow = arrow(length = unit(0.2, "cm")),
        ...
        scale_size_manual(values = c("N" = 1.5 * abs(net_N), "C" = 1.5 * abs(net_C))) +
        theme_transparent() +
